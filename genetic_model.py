...
def cal_pop_fitness(equation_inputs, pop, real_value):
	...
	for i in range(len(pop)):
		...
		for j in range(len(equation_inputs)):
			guess = (2/40)*x[j][0]*w[i][0] + (3/40)*x[j][1]*w[i][1] + (4/40)*x[j][2]*w[i][2] + (7/40)*x[j][3]*w[i][3] + (10/40)*x[j][4]*w[i][4] + (12/40)*x[j][5]*w[i][5]
...
new_population = np.random.uniform(low=0.0, high=20.0, size=pop_size)
...
for j in range(len(x)):
	result += [(2/40)*x[j][0]*w[0] + (3/40)*x[j][1]*w[1] + (4/40)*x[j][2]*w[2] + (7/40)*x[j][3]*w[3] + (10/40)*x[j][4]*w[4] + (12/40)*x[j][5]*w[5]]
